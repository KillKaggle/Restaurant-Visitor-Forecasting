...
air_visit <- fread("E:/restaurantVisitor/数据/restaurant/air_visit_data.csv")
...
air_reserve <- fread("E:/restaurantVisitor/数据/restaurant/air_reserve.csv")
air_reserve[, date:=as.character(as.Date(visit_datetime))]

date_info <- fread("E:/restaurantVisitor/数据/restaurant/date_info.csv")
...
air_store_info <- fread("E:/restaurantVisitor/数据/restaurant/air_store_info.csv", drop=3)
...
submission <- fread("E:/restaurantVisitor/数据/restaurant/sample_submission.csv", select=1)
submission <- merge(submission, prediction, by="id", all.x=TRUE)
submission[visitors < 1, visitors:=1]
write.csv(submission, file.choose("E:/sample_submission.csv"),row.names=FALSE)
